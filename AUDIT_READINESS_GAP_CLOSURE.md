...
| G2 | Dependency lock governance not formalized | `requirements-lock.txt` exists | Publish lock refresh SOP (monthly + emergency CVE path) and enforce PR check for lock drift | Engineering Lead | SOP doc, 2 approved change tickets, PR check screenshot | High | Open |
...
| G8 | Threat model traceability missing from final audit pack | OWASP traceability exists | Add lightweight threat model for auth, uploads, petition workflow transitions, and admin actions | AppSec | Threat model document, review sign-off, mitigation mapping | Medium | Open |
| G9 | IR playbook linkage to app-specific events not evidenced | Security monitoring policy exists | Map each key event type to incident severity, responder role, and SLA | SecOps | IR runbook extract, escalation matrix, tabletop drill evidence | Medium | Open |
| G10 | Auditor-ready evidence index not centralized | Docs are present in repo | Create single audit evidence index with source path, owner, and last-verified date | Compliance | Evidence index sheet, owner attestations, QA checklist | Medium | Open |
